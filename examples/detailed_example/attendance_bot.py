...
time_start = []
time_end = []
@bot.message_handler(commands=['reporting'])
def command_start_working(m):
    cid = m.chat.id
    name = m.chat.username
    print(m.text)
    msg = bot.reply_to(m, "please tell me your check in time for Monday")
    bot.register_next_step_handler(msg, working_day1)
...
def working_day1(m):
    try:
        ...
        if m.text == 'sick' or m.text == 'vacation':
            time_start.append(m.text)
            time_end.append(m.text)
            print(time_start)
            print(time_end)
            msg = bot.reply_to(m, "please tell me your check in time for Tuesday")
            bot.register_next_step_handler(msg, working_day2)
        else:
            monday = datetime.strptime(m.text, '%Y-%m-%d %H:%M')
    # monday.strftime('%Y-%m-%d %H:%M')
            time_start.append('%s' % monday)
            monday_end = monday + timedelta(hours=8)
    # monday_end.strftime('%Y-%m-%d %H:%M')
            time_end.append('%s' % monday_end)
        ...
        print(e)
        msg = bot.reply_to(m, "error,please tell me your check in time for Monday")
        bot.register_next_step_handler(msg, command_start_working)
...
def working_day2(m):
    try:
        ...
        if m.text == 'sick' or m.text == 'vacation':
            ...
            print(time_start)
            print(time_end)
            msg = bot.reply_to(m, "please tell me your check in time for Wednesday")
            bot.register_next_step_handler(msg, working_day3)
        else:
            tuesday = datetime.strptime(m.text, '%Y-%m-%d %H:%M')
    # monday.strftime('%Y-%m-%d %H:%M')
            time_start.append('%s' % tuesday)
            tuesday_end = tuesday + timedelta(hours=8)
    # monday_end.strftime('%Y-%m-%d %H:%M')
            time_end.append('%s' % tuesday_end)
...
def working_day3(m):
    try:
        ...
        if m.text == 'sick' or m.text == 'vacation':
            ...
            print(time_start)
            print(time_end)
            msg = bot.reply_to(m, "please tell me your check in time for Thrusday")
            bot.register_next_step_handler(msg, working_day4)
        else:
            wednesday = datetime.strptime(m.text, '%Y-%m-%d %H:%M')
    # monday.strftime('%Y-%m-%d %H:%M')
            time_start.append('%s' % wednesday)
            wednesday_end = wednesday + timedelta(hours=8)
    # monday_end.strftime('%Y-%m-%d %H:%M')
            time_end.append('%s' % wednesday_end)
...
def working_day4(m):
    try:
        ...
        if m.text == 'sick' or m.text == 'vacation':
            ...
            print(time_start)
            print(time_end)
            msg = bot.reply_to(m, "please tell me your check in time for Friday")
            bot.register_next_step_handler(msg, working_day5)
        else:
            thrusday = datetime.strptime(m.text, '%Y-%m-%d %H:%M')
    # monday.strftime('%Y-%m-%d %H:%M')
            time_start.append('%s' % thrusday)
            thrusday_end = thrusday + timedelta(hours=8)
    # monday_end.strftime('%Y-%m-%d %H:%M')
            time_end.append('%s' % thrusday_end)
...
def working_day5(m):
    try:
        ...
        if m.text == 'sick' or m.text == 'vacation':
            ...
            friday = datetime.strptime(m.text, '%Y-%m-%d %H:%M')
    # monday.strftime('%Y-%m-%d %H:%M')
            time_start.append('%s' % friday)
            friday_end = friday + timedelta(hours=8)
    # monday_end.strftime('%Y-%m-%d %H:%M')
            time_end.append('%s' % friday_end)
            msg = bot.reply_to(m, "Thanks")
            print(time_start)
            print(time_end)
    except Exception as e:
        msg = bot.reply_to(m, "error,please tell me your check in time for Monday")
    ...
    # bot.register_next_step_handler(msg, listen_time)    # bot.send_message(cid, "please tell me your ")
    # now = datetime.now()
